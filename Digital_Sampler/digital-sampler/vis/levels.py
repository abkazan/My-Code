...
class DbMeter(tk.Canvas):
    ...
    def __init__(self, master: tk.Misc = None, __scale: float = 0.5,
                 *args, **kwargs):
        '''
    ...
    def _update_volumes(self):
        ...
        for i, chan in enumerate(self.__channels):
            ...
            # max
            if chan.audio.frames <= chan.frame_num + elapsed_frames:
                pass
            else:
    ...
    def assign(self, audio: AudioFile, channel_num: int) -> None:
        ...
        if self.__channels[channel_num]:
            self.__channels[channel_num].audio.close()
            del self.__channels[channel_num].audio
